...
module.exports = function(state) {
  ...
  return function(options, onError) {
    var modelPlugins = plugins({
      pattern: 'statictemp-model-*',
      replaceString: /^statictemp-model-/,
      apply: false
    });
    for (var pluginName in modelPlugins) {
      ...
      if (modelPlugins.hasOwnProperty(pluginName)) {
        ...
        Object.defineProperty(ContentModel.prototype, pluginName, {
          get: function() {
            var model = this;
            return modelPlugins[pluginName].call(
              model,
              options,
              state.contentList
            );
    ...
    return fs
      .src(path.join('**', options.templateAdapter.templateGlob), {
        cwd: options.content
      })
      .pipe(
        es.map(function(file, cb) {
          ...
          var parsedPath = parsePath(
            relativePath,
            options.templateAdapter.templateExtension
          );
